...
VBuffer::VBuffer(const VDevice& device, const VBufferInfo& bufferInfo)
    : m_Device(device)
{
...
bool VBuffer::createBuffer(const VBufferInfo& bufferInfo, VkBuffer& buffer, VkDeviceMemory& memory)
{
...
bool VBuffer::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags flags, uint32_t& typeIndex)
{
...
// ----------- VVertexBuffer --------------
VVertexBuffer::VVertexBuffer(const VDevice& device, const std::vector<Vertex>* vertices, const std::vector<uint16_t>* indices)
    : VBuffer(device), m_Vertices(vertices),
      m_Indices(indices)
{
    createVertexBuffer();
    createIndexBuffer();
